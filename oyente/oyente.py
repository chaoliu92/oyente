...
def analyze_bytecode(trace_file):
    ...
    # in case we are given a guided trace
    trace = None
    if trace_file is not None:
        with open(trace_file) as f:
            trace = json.load(f)
        trace = [item['PC'] for item in trace]
        trace.reverse()

    result, exit_code = symExec.run(disasm_file=inp['disasm_file'], trace=trace, helper=helper)
    helper.rm_tmp_files()
...
def run_solidity_analysis(inputs):
    ...
    for inp in inputs:
        ...
        result, return_code = symExec.run(disasm_file=inp['disasm_file'], source_map=inp['source_map'],
...
def analyze_solidity(input_type='solidity'):
    ...
    if input_type == 'solidity':
        helper = InputHelper(InputHelper.SOLIDITY, source=args.source, evm=args.evm,
                             compilation_err=args.compilation_error, root_path=args.root_path, remap=args.remap,
...
def main():
    ...
    parser.add_argument("-rp", "--root-path", help="Root directory path used for the online version", action="store",
                        dest="root_path", type=str)
    ...
    exit_code = 0
    if args.bytecode:
        exit_code = analyze_bytecode(args.trace_file)
